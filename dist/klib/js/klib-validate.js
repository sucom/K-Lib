...
function _fnValidWordSize(obj, msg) {
  var elValue = $(obj).val();
  var maxWordSize = _.max(_.map((''+elValue).replace(/-/g,' ').split(' '), function(w) { return w.length; }));
...
function _fnValidIpAddress(obj, msg) {
  ...
  { var strIpAddr = elValue;
    ...
    var isValid = ((rxValidIp.test(strIpAddr)) && ( _.every(strIpAddr.split('.'), function(part){ return ((part.length>1)? (!part.beginsWith("0")) : true); }) ));
    _showValidateMsg(obj, msg, isValid);
...
klib.initDataValidation = function(context){
  ...
  //console.log(_.keys(commonValidateRules));
  ...
  $context.find(elSelector+"[data-validate]").each(function(index, el){
    ...
    //console.log(elValidateRuleSpec);
    ...
    _.each(_.keys(elValidateRules), function(validateOnEvent){
      ...
      //console.log("settingup event on["+jqEventName+"]...");
